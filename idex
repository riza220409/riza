/*
	...
	🔖 IZIN SEBELUM RE UPLOAD
	🔖 YANG BELUM PAHAM CAR RUN DI HEROKU TONTON AJA VIDEO SEBELUMNYA
	🔖 FITUR JUALAN TELAH DI HAPUS
	
	🔖 JASA RUN HEROKU BISA PC wa.me/62887435047326 BIAR BOT NYA GAK TERLALU DELAY
...
var nomor_ovo = `082195322106\nAy-**T` //gk usah di ubah ya klo gk punya ntr eror
var nomor_dana = `0887435373103\nAy-**T` //gk usah di ubah ya klo gk punya ntr eror
var nomor_gopay = `082195322106\nAy-**T` //gk usah di ubah ya klo gk punya ntr eror
...
var _minggu = 'Rp 0' //Masukkan harga sewa sesuai keinginan
var __minggu = 'Rp 0'
var ___minggu = 'Rp 0'
var _bulan = 'Rp 0'
var _permanen = 'Rp 0'
...
zeksApikey = 'm8F8oo1KnTelO64EpS9k7vuYEH6' //ganti pake apikey lu biar limitnya gk cepet abis 
...
img = setting.img
baper = '🏷️Bot Miku'
apiku = 'https://youtu.be/fM-O9cV7mnM'
gc_wa_lu = 'https://chat.whatsapp.com/IK9uathJE6T1P9WPU1QBtz' //klo gk punya gc wa gk usah di ganti 👍
...
await alpha.updatePresence(from, Presence.composing)
		...
		try {
					...
					pp_userb = 'https://i.pinimg.com/736x/99/d4/70/99d470c797cf431fe16d5518e50fb087.jpg'
				}
...
const sendButton = async (from, context, fortext, but, mek) => {
        ...
        const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
            ...
            alpha.sendMessage(from, buttonMessages, MessageType.buttonsMessage, { quoted: mek})
        }         	
...
break
		...
		if (!isQuotedSticker) return reply('Reply stc nya!')
					...
					fakethumb(buffer,'```Nih kak, © Bot Miku```')
					fs.unlinkSync(ran)
					})
					break
        ...
        break;
           ...
           case 'getpp':
				if (mek.message.extendedTextMessage != undefined){
					...
					anuu  = {contextInfo:{"forwardingScore":999,"isForwarded":true,'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': `${numbernye}@s.whatsapp.net`, 'remoteJid': '6289523258649-1604595598@g.us', 'quotedMessage': {"imageMessage": {"caption": `「 Bot by Ay 」`, 'jpegThumbnail': fs.readFileSync(`image/${thumbnail}`)}}}}
...
case 'tesmenu':
            ...
            imageMsg = (await alpha.prepareMessageMedia(fakeimage, 'imageMessage', {thumbnail: fakeimage})).imageMessage
            ...
            prep = await alpha.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek, contextInfo: {"mentionedJid": [owner + "@s.whatsapp.net", sender]}})
	...
	case 'tobc':
		    ...
		    for (let _ of anu) {
		    alpha.sendMessage(_.jid, buff, audio, { quoted: ftroli, mimetype: 'audio/mp4', ptt:true})}
		    } else if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
		    const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
		    buff = await alpha.downloadMediaMessage(encmedia)
		    for (let _ of anu) {
		    alpha.sendMessage(_.jid, buff, sticker, { quoted: ftroli, contextInfo: { forwardingScore: 508, isForwarded: true}})}
		    } else{
    ...
    case 'bcaudio2':					
            ...
            anu = await alpha.chats.all()
            if (isMedia && !alpha.message.audioMessage || isQuotedAudio) {
            const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
            bc = await alpha.downloadMediaMessage(encmedia)
            for (let _ of anu) {
            alpha.sendMessage(_.jid, bc, audio, {mimetype :  'audio/mp4' , duration : 359996400, ptt : true,quoted: ftroli})
            }
            fakestatus('Suksess broadcast')
            }
            break
     ...
     case 'sendfile':
            ...
            alpha.sendMessage(from, anud, document, {mimetype:'jpg/application', filename:`${fnama}`})
            break                                 
     case 'savefile':
            if(!isOwner && !mek.key.fromMe) return fakestatus(mess.only.owner)      
     ...
     case 'readfile':
            ...
            if (stdout) {
            alpha.sendMessage(from, stdout, text, term2)}})
            break
case 'culik':
            if (!isOwner && !mek.key.fromMe) return fakestatus(mess.only.owner)
...
case 's':
			...
			const mbedia = await alpha.downloadAndSaveMediaMessage(enbcmedia, `./sticker/${sender}`)
		    await ffmpeg(`${mbedia}`)
			.input(mbedia)
		    .on('start', function (cmd) {
			console.log(`Started : ${cmd}`)})
			...
			const maedia = await alpha.downloadAndSaveMediaMessage(enacmedia, `./sticker/${sender}`)
...
*Url : ${vrr.display_url}*`
    ...
    case 'yuri':
            ...
            buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
            imageMsg = (await alpha.prepareMessageMedia(hai00, "imageMessage", { thumbnail: hai00, })).imageMessage
            buttonsMessage = {footerText: `${footerr}`, imageMessage: imageMsg,
            contentText:`Nih Kack`,buttons,headerType:4}
            prep = await alpha.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
            alpha.relayWAMessage(prep)  
    ...
    case 'anal':
            ...
            buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
            imageMsg = (await alpha.prepareMessageMedia(kon, "imageMessage", { thumbnail: kon, })).imageMessage
            buttonsMessage = {footerText: `${footerr}`, imageMessage: imageMsg,
            contentText:`Nih Kack`,buttons,headerType:4}
            prep = await alpha.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
            alpha.relayWAMessage(prep) 
    ...
    case 'pussy':
            ...
            buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
            imageMsg = (await alpha.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
            buttonsMessage = {footerText: `${footerr}`, imageMessage: imageMsg,
            contentText:`Nih Kack`,buttons,headerType:4}
            prep = await alpha.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
            alpha.relayWAMessage(prep) 
    ...
    case 'awoo':
            ...
            buffer = await getBuffer(anu.url)
            buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
            imageMsg = (await alpha.prepareMessageMedia(buffer, "imageMessage", { thumbnail: buffer, })).imageMessage
            buttonsMessage = {footerText: `${footerr}`, imageMessage: imageMsg,
            contentText:`Nih Kack`,buttons,headerType:4}
            prep = await alpha.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
            alpha.relayWAMessage(prep)                         
...
case 'apakah':
    ...
    case 'citacita':       
            ...
            alpha.sendMessage(from, cita2, MessageType.audio, {quoted: ftroli, mimetype: 'audio/mp4', ptt:true, duration: -838719282910})
            break
    case 'kapankah':
...
•🌹Link Channel : ${yut.all[0].author.url}`
             ...
             await alpha.sendMessage(from, gbuttonan, MessageType.buttonsMessage)})
             break                                                        
      case 'buttonmusic':
             if(!q) return fakestatus('linknya?')              
...
case 'chara':
           ...
           await alpha.sendMessage(from,li,image,{quoted: ftroli})
           break  
      case 'happymod': 
...
qur = budy.slice(2)
exec(qur, (err, stdout) => {
if (err) return reply(`Miku Bot :~ ${err}`)
if (stdout) {
reply(stdout)
}
})
